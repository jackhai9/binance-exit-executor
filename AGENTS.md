...
- 不要添加 silent default、best-effort 分支、宽泛 `try/except`、兼容 shim、旧字段解析、旧路径识别、自动 retry、类型猜测转换或随手 `None` 处理来让流程继续运行。
- 交易执行、风控约束、订单状态机、WebSocket、配置契约和通知控制遇到非法状态时应快速失败，暴露清晰错误；只有当前代码契约或用户当轮明确要求迁移支持时，才允许保留 recovery 或兼容路径。
- 如果输入、配置、交易所响应、仓位数据、时序或上游行为不确定，先读取事实源并收紧契约；不要用猜测出来的默认值、防御分支或兼容层覆盖不确定性。
- recovery 代码只允许出现在已定义恢复语义的边界上，必须窄化捕获范围，写清预期失败模式，并用具体断言测试恢复路径。
